...
# Run the current best ETTh2 configurations found during fixed-seed tuning.
# These are not the old GitHub/comment configs. They include the exposed
# training knobs that produced the current best H2 metrics.
...
run_h2() {
  ...
  local model_id="$2"
  local des="$3"
  local log_path="${RUN_ROOT}/logs/ETTh2_${pred_len}.log"
  shift 3
  echo "START ETTh2-${pred_len} log=${log_path}"
  python -u run_can.py \
    "${common_args[@]}" \
    --pred_len "${pred_len}" \
    --model_id "${model_id}" \
...
run_h2 96 ETTh2_96_r275_0000 ETTh2_h2_best_r275_0000 \
...
run_h2 192 ETTh2_192_r305_0000 ETTh2_h2_best_r305_0000 \
...
run_h2 336 ETTh2_336_r310_0002 ETTh2_h2_best_r310_0002 \
...
run_h2 720 ETTh2_720_r68_0006 ETTh2_h2_best_r68_0006 \
